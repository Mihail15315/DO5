...
* **Остановка базового контейнера:**
  ...
  * *Скриншот остановки процесса контейнера:*
  * ![Остановка контейнера и проверка через docker ps](/screens/screen_stop.png)
    *(На скриншоте зафиксирован вызов команды остановки по ID и пустой вывод docker ps, подтверждающий успешное завершение работы).*

* **Публикация портов и проверка веб-сервера:**
  ...
  * *Скриншот запуска с портами и приветственной страницы:*
  * ![Запуск контейнера с маппингом портов и проверка доступности страницы](/screens/screen_localhost_80.png)
    *(На скриншоте показана команда запуска с флагами -p, а ниже — вывод утилиты curl по адресу localhost:80, где отчетливо виден успешный ответ "Welcome to nginx!" и сам запрашиваемый URL).*
